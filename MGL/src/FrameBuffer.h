...
namespace mgl {
	...
	public:
		...
		FrameBufferProperties(const uint32_t width, const uint32_t height, const std::initializer_list<TextureProperties> attachments) :
	...
	public:
		...
		inline uint32_t GetColourAttachmentIdAt(const uint32_t index = 0) const { MGL_ASSERT(index < m_ColourAttachmentIds.size(), "Index outside bounds of colour attachments."); return m_ColourAttachmentIds[index]; }
		inline uint32_t GetDepthAttachmentId() const { return m_DepthAttachmentId; }
		inline void SetProperties(const FrameBufferProperties newProps) { m_Properties = newProps; Invalidate(); }
		inline void SetReadOnly(const bool readOnly) { Bind(); if (readOnly) glReadBuffer(GL_NONE); else glReadBuffer(GL_ONE); Unbind(); }
		inline void SetDrawBuffer(const int attachmentId = 0) { Bind(); glDrawBuffer(GL_COLOR_ATTACHMENT0 + attachmentId); Unbind(); }
	};
}
